-- FIX ALL: Feature Requests RLS and RPC
-- Run this in Supabase SQL Editor to resolve all permission issues

-- 1. Reset and Enable RLS for feature_requests
ALTER TABLE public.feature_requests ENABLE ROW LEVEL SECURITY;

-- Allow ALL authenticated users to do EVERYTHING (simplest policy for troubleshooting)
...
-- Also ensure profiles are readable (needed for reporter relationship)
ALTER TABLE public.profiles ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "Enable read access for authenticated users" ON public.profiles;
CREATE POLICY "Enable read access for authenticated users" ON public.profiles
    FOR SELECT TO authenticated USING (true);

-- 2. Create/Replace the RPC function as a backup
-- This function runs with SECURITY DEFINER (Admin privileges), bypassing RLS
